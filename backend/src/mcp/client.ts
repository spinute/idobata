...
function convertMcpToolsToOpenAI(mcpTools: McpTool[]): OpenAI.Chat.ChatCompletionTool[] {
...
export class McpClient {
    ...
    private _initialized: boolean = false; // Track initialization status
    ...
    async connectToServer(serverScriptPath: string): Promise<void> {
        ...
        try {
            ...
            this._tools = toolsResult.tools.map((tool: any) => ({ // Simplified map return
                name: tool.name,
                description: tool.description,
    ...
    async processQuery(
        ...
        const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
            {
                role: 'system',
                content: `あなたは、ユーザーが政策案の改善提案を作成するのを支援するAIアシスタントです。「現在のファイル内容」として提供された政策文書について、ユーザーが改善提案を練り上げるのを手伝うことがあなたの目標です。
...
2.  **下書きの編集:** 具体的な変更点について合意したら、利用可能なツールを使って、これらの変更をあなた専用の下書きスペース（ブランチ）に新しいバージョン（コミット）として保存します。これはユーザーの個人的な編集作業です。変更点の要約を提示して最終確認を求め、承認を得たら直接変更をコミットします。変更箇所は本当に必要なものだけにしてください。
    この作業は、ユーザーが望む限り何度でも繰り返せます。
3.  **レビュー依頼の準備:** 下書きの編集にユーザーが満足したら、他の人にフィードバックをもらうためにレビューを依頼する準備をします。行われた改善点を説明する明確な要約メッセージを作成して、そのまま利用可能なツールを使って送信してください。このメッセージは、変更内容とその理由を他の人に伝えるために使われます（プルリクエスト）。
4.  **リンクの共有:** 最後に、提案された変更へのウェブリンク（プルリクエストリンク）をユーザーに提供してください。
                ...
                if (message.tool_calls && message.tool_calls.length > 0) {
                    ...
                    for (const toolCall of message.tool_calls) {
                        ...
                        try {
                            ...
                            logger.error(`Failed to parse arguments for tool ${toolName}:`, parseError);

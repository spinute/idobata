...
3.  **レビュー依頼の準備:** 下書きの編集にユーザーが満足したら、他の人にフィードバックをもらうためにレビューを依頼する準備をします。利用可能なツールを使ってプルリクエストのタイトルと説明を設定してください。説明では、行われた改善点、その意図や目的、背景などを可能な限り明確かつ詳細に述べ、ユーザー名（${userName || '匿名'}）を記載しましょう。このメッセージは、変更内容とその理由を他の人に伝えるために使われます（プルリクエスト）。

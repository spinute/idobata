...
3.  **レビュー依頼の準備:** 下書きの編集にユーザーが満足したら、他の人にフィードバックをもらうためにレビューを依頼する準備をします。行われた改善点を説明する明確な要約メッセージを作成し、ユーザー名（${userName || '匿名'}）を記載しましょう。その後、利用可能なツールを使ってプルリクエストのタイトルと文章を設定してください。このメッセージは、変更内容とその理由を他の人に伝えるために使われます（プルリクエスト）。

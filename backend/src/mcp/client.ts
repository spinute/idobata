...
3.  **レビュー依頼の準備:** 下書きの編集にユーザーが満足したら、他の人にフィードバックをもらうためにレビューを依頼する準備をします。行われた改善点を説明する明確な要約メッセージを作成し、ユーザー名（${userName || '不明'}）を記載して、そのまま利用可能なツールを使って送信してください。このメッセージは、変更内容とその理由を他の人に伝えるために使われます（プルリクエスト）。

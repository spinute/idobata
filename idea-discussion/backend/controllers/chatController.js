...
const handleNewMessage = async (req, res) => {
  try {
    ...
    try {
      ...
      if (allQuestions.length > 0) {
        ...
        for (const question of allQuestions) {
          ...
          const problemLinks = await QuestionLink.aggregate([
            ...
            }, // Deconstruct the array, keep if no match
          ]);
      ...
      console.error(
        "Error fetching reference opinions (questions/links):",
        dbError
      );
...
`;
    ...
    setTimeout(() => {
      ...
      processExtraction(job).catch((err) => {
        console.error(
          `[Async Extraction Call] Error for thread ${chatThread._id}:`,
          err
        );
      });
    }, 0);

...
async function linkQuestionToAllItems(questionId) {
  console.log(
    `[LinkingWorker] Starting linking for new Question ID: ${questionId}`
  );
  try {
    ...
    console.log(
      `[LinkingWorker] Linking Question ${questionId} to ${problems.length} problems and ${solutions.length} solutions.`
    );

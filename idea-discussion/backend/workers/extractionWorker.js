...
async function saveAndLinkItem(
  ...
  if (itemData.type === "problem") {
    ...
    console.log(
      `[ExtractionWorker] Added Problem: ${savedItem._id} from ${sourceType} ${sourceOriginId}`
    );
    ...
    console.log(
      `[ExtractionWorker] Added Solution: ${savedItem._id} from ${sourceType} ${sourceOriginId}`
    );
  }
    ...
    if (sourceType === "chat") {
      ...
      for (let i = 0; i < additions.length; i++) {
        ...
        const savedItem = await saveAndLinkItem(
          ...
          "chat",
          {}
        ); // No specific metadata for chat items
        if (savedItem) {
